...
def accept_parser():
    ...
    parser.add_argument('--province', '-pr', help='省份 属性，非必要参数，必须要填全名（如宁夏回族自治区），不填默认为在学校', default='', required=False)
    parser.add_argument('--city', '-c', help='城市 属性，非必要参数，必须要填全名（如合肥市）', default='', required=False)
    parser.add_argument('--district', '-d', help='区县 属性，非必要参数，必须要填全名（如长安区）', default='', required=False)
    parser.add_argument('--detailed', '-de', help='详细居住地 属性，非必要参数，如住在西安且不在学校需填写现居住地址（##街道##小区#号楼#单元#室）', default='', required=False)
